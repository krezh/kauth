...
func TestRateLimiter_DifferentIPsDoNotShareLimit(t *testing.T) {
	rl := NewRateLimiter(1, 1, time.Minute)
	handler := rl.Middleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	...
	if rr.Code != http.StatusOK {
		t.Fatalf("first request from IP1 should pass, got %d", rr.Code)
	}
	...
	if rr.Code != http.StatusOK {
		t.Errorf("first request from different IP2 should pass, got %d", rr.Code)
	}
}
...
func TestRateLimiter_SameClientIPSharesLimit(t *testing.T) {
	rl := NewRateLimiter(1, 1, time.Minute)
	handler := rl.Middleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	...
	handler.ServeHTTP(rr, req2)
	if rr.Code != http.StatusTooManyRequests {
		t.Errorf("same client IP with different proxy should share limit, got %d", rr.Code)
	}
}