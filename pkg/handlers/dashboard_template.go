...
var dashboardTemplate = template.Must(template.New("dashboard").Funcs(template.FuncMap{
	...
	"when": func(value time.Time) template.HTML {
		if value.IsZero() {
			return "Never"
		}
		iso := value.UTC().Format(time.RFC3339)
		fallback := value.UTC().Format("2006-01-02 15:04:05Z")
		return template.HTML(fmt.Sprintf(`<time datetime="%s">%s</time>`, iso, fallback))
	},
...
{{define "events-tbody"}}<tbody id="events-tbody">{{if .Events}}{{range $i, $g := .Events}}<tr{{if $g.Discovery}} class="row-toggle" data-group="g{{$i}}"{{end}}><td class="cell-muted">{{if $g.Discovery}}<span class="chevron">&#9656;</span>{{end}}{{when $g.OccurredAt}}</td><td class="request-method">{{$g.Method}}</td><td class="path">{{$g.Path}}{{if $g.Discovery}}<span class="count-badge">+{{len $g.Discovery}}</span>{{end}}</td><td class="status-{{statusClass $g.StatusCode}}">{{$g.StatusCode}}</td><td>{{duration $g.Duration}}</td><td>{{bytes $g.ResponseBytes}}</td></tr>{{range $g.Discovery}}<tr class="discovery-row" data-group="g{{$i}}"><td class="cell-muted">{{when .OccurredAt}}</td><td class="request-method">{{.Method}}</td><td class="path">{{.Path}}</td><td class="status-{{statusClass .StatusCode}}">{{.StatusCode}}</td><td>{{duration .Duration}}</td><td>{{bytes .ResponseBytes}}</td></tr>{{end}}{{end}}{{else}}<tr><td colspan="6" class="empty">No API requests recorded for this session.</td></tr>{{end}}</tbody>{{end}}
