...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	resp, err := http.Get(serviceURL + "/info")
	if err != nil {
	...
	// Start login flow
	loginResp, err := http.Get(serviceURL + "/start-login")
	if err != nil {
	...
	kubeconfig, err := watchForCompletion(serviceURL, loginData.SessionID)
	if err != nil {
		return err
	}
...
func watchForCompletion(baseURL, sessionID string) (string, error) {
	resp, err := http.Get(fmt.Sprintf("%s/watch?session=%s", baseURL, sessionID))
	if err != nil {
